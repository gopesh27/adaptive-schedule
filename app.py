...
if uploaded_file is not None:
    # 👇 Auto-handle decimal comma or dot
    try:
    ...
    input_cols = st.multiselect(
        ...
        all_columns, 
        default=[c for c in all_columns if c.lower() not in ["machine", "manpower"]]
    )
    output_cols = st.multiselect(
        "Select Output Columns (y)", 
        all_columns, 
        default=[c for c in all_columns if c.lower() in ["machine", "manpower"]]
    )
...
if "model" in st.session_state:
    ...
    for col in input_cols:
        if df[col].dtype in ["int64", "float64"]:
            ...
            # 👇 Force dot decimal regardless of locale
            input_data[col] = float(str(val).replace(",", "."))
        else:
